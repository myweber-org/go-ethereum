...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage:")
		fmt.Println("  Generate key: file_encryption -genkey")
		fmt.Println("  Encrypt: file_encryption -encrypt <input> <output> <hex_key>")
		fmt.Println("  Decrypt: file_encryption -decrypt <input> <output> <hex_key>")
		return
	}
	...
	case "-encrypt":
		if len(os.Args) != 5 {
			fmt.Println("Usage: file_encryption -encrypt <input> <output> <hex_key>")
			os.Exit(1)
		}
	...
	case "-decrypt":
		if len(os.Args) != 5 {
			fmt.Println("Usage: file_encryption -decrypt <input> <output> <hex_key>")
			os.Exit(1)
		}

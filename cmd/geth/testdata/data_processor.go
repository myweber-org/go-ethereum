...
func parseRecord(row []string, lineNum int) (DataRecord, error) {
...
func ValidateRecords(records []Record) (valid []Record, invalid []Record) {
	for _, record := range records {
...
func CalculateStatistics(records []Record) (sum float64, average float64, count int) {
	if len(records) == 0 {
		return 0, 0, 0
	}

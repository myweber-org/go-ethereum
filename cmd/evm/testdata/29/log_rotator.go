...
func (lr *LogRotator) compressBackup(filename string) error {
    ...
    target, err := os.Create(compressed)
    if err != nil {
        return err
    }
    defer target.Close()
...
func (lr *LogRotator) cleanupOldBackups() error {
    ...
    for i := 0; i < len(matches)-maxBackups; i++ {
        ...
        for j := 1; j < len(backupTimes); j++ {
            if !backupTimes[j].IsZero() && backupTimes[j].Before(backupTimes[oldestIndex]) {
                oldestIndex = j
            }
        }
        os.Remove(matches[oldestIndex])
        backupTimes[oldestIndex] = time.Time{}
    }
...
func (rl *RotatingLogger) rotateBySize() error {
	...
	srcPath := rl.basePath + ".log"
	destPath := fmt.Sprintf("%s.%s.log.gz", rl.basePath, time.Now().Format("20060102150405"))

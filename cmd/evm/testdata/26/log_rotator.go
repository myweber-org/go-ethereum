...
func (rl *RotatingLogger) rotate() error {
	...
	rl.rotationCount++
	archiveName := fmt.Sprintf("%s.%d-%s.gz", rl.basePath, rl.rotationCount, time.Now().Format("20060102T150405"))
...
func (rl *RotatingLogger) rotate() error {
    ...
    if err := os.Remove(rl.filename); err != nil && !os.IsNotExist(err) {
        return err
    }
...
func cleanupOldBackups(baseFilename string) error {
    pattern := filepath.Join(filepath.Dir(baseFilename), 
        filepath.Base(baseFilename)+".*.gz")
